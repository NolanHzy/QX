...
^https://api.1jainlian.xyz:21987/api/app/auth/user/profile url script-request-body https://raw.githubusercontent.com/NolanHzy/QX/main/Script/YJLAPP.js
...
let isrequest = typeof $request !== 'undefined'

function CheckApi(){
    if($request.url.indexOf("t.captcha.qq.com/cap_union_new_show")>-1){
   ...
   if($request.url.indexOf("t.captcha.qq.com/cap_union_new_verify")>-1){
    ...
    if($request.url.indexOf("/api/app/auth/user/profile")>-1){
        var rqbody = $request.body;
        var resul= $response.body;
        console.log(rqbody);
        $.log(rqbody)
